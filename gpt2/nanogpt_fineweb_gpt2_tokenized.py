...
class RandomBatchLoader:
    """
    On each next_batch(), samples B independent random (contiguous) chunks of length T.
    """
    def __init__(self, data_dir: str, split: str, B: int, T: int, num_chunks: Optional[int]):
        files = list_split_bins(data_dir, split, num_chunks)
        self.sampler = BinChunkSampler(files, T)
        self.B, self.T = B, T
        print(f"[{split}] files={len(files)} | tokens={sum(self.sampler.lengths):,} | T={T}")

    def next_batch(self, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        xs, ys = [], []
        for _ in range(self.B):
            x, y = self.sampler.sample_one()
            xs.append(x)
            ys.append(y)
        x = torch.tensor(np.stack(xs, axis=0), dtype=torch.long, device=device)  # (B, T)
        y = torch.tensor(np.stack(ys, axis=0), dtype=torch.long, device=device)  # (B, T)
        return x, y
...
def main():
    ...
    p.add_argument("--n_embed", type=int, default=384)
    ...
    # loaders
    train_loader = RandomBatchLoader(args.data_dir, "train", args.batch_size, args.block_size, args.num_train_chunks)
    val_loader   = RandomBatchLoader(args.data_dir, "val",   args.batch_size, args.block_size, args.num_val_chunks)
    ...
    for step in range(1, args.max_iters + 1):
        ...
        for micro in range(1, micro_in_macro + 1):
            ...
            ctx = torch.autocast(device_type=device.type, dtype=amp_dtype) if use_amp else torch.cuda.amp.autocast(enabled=False)
            with ctx:
                _, loss = model(x, y)
        ...
        # console ping
        if (step % max(1, args.eval_interval // 10)) == 0 or step == 1:
            print(f"step {step:6d} | train_loss (last micro) {loss.item():.4f} | eff_tok/s {eff_toks_per_sec:,.0f}")

        # wandb ping
        if run:
            wandb.log(
                {
                    "train/loss_last_micro": float(loss.item()),
        ...
        if (step % args.eval_interval) == 0 or step == args.max_iters:
            ...
            print(f"[eval] step {step:6d} | eval_loss {eval_loss:.4f} | elapsed {elapsed:.1f}s")
            if run:
                wandb.log({"eval/loss": float(eval_loss), "timing/elapsed_sec": elapsed, "step": step}, step=step)
