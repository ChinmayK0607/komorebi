...
# against the same public step-512 baseline.
set -Eeuo pipefail
RUN="$(cd "${1:?usage: launch_astra_firstpaint_eval.sh RUN_DIRECTORY baseline|trained}" && pwd -P)"
...
PY
  ...
  ADAPTER="$RUN/train-output/astra-firstpaint-sft-20260926-v1/artifacts/adapters/step_$FINAL_STEP"
fi
...
"$PY" "$RUN/painter/native_painting_eval.py" \
  --root "$RUN/painter" --manifest "$MANIFEST" --policy-label "astra-firstpaint-sft-$POLICY" \
...
receipt={'schema':'painter.astra-firstpaint-matched-eval.v1','status':'completed',
