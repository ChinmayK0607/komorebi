...
STAGE="$($PY "$RUN/prepare_mimo_cloud.py" --tier easy --model xiaomi/mimo-v2.6-pro | $PY -c 'import json,sys; print(json.load(sys.stdin)["stage_root"])')"
"$PY" - "$STAGE" <<'PY'
import hashlib, json, pathlib, sys
root = pathlib.Path(sys.argv[1])
path = root / 'config.json'
config = json.loads(path.read_text())
config['benchmark'] = 'quality-curriculum-pro-cup-opaque-20260925'
config['tracks']['quality'].update(max_turns=4, max_tokens=16384, episode_timeout_seconds=2400)
...
source['probe'] = {'reference_id': 'simple-cup', 'comparison': 'Flash underpaint canary at turn 4',
                   'max_turns': 4, 'max_completion_tokens': 16384,
...
"$PY" "$BENCH/cloud/publish_results.py" --root "$STAGE" --run-id mimo-pro-cup-opaque-20260925
