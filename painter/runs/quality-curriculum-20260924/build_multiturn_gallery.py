...
NODE_ROOT = Path("/root/painter/multiturn-run")
...
def build(evidence: Path, output: Path) -> dict:
    ...
    summary = " · ".join(f'{policy}: {counts[policy]["traces"]}/{len(cases)} cases, {counts[policy]["final_valid"]} valid final canvases'
                         for policy in POLICIES)
    page = ('<!doctype html><html><head><meta charset="utf-8"><title>Multi-turn painting SFT comparison</title>'
            ...
            'h2 small,h3 small{font-weight:400;color:#a8b4b6}.columns{display:grid;grid-template-columns:repeat(4,minmax(260px,1fr));gap:12px}'
